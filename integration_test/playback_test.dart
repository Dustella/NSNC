...
void main() {
  ...
  test(
    'resolves a live URL and plays audio (position advances)',
    () async {
      final client = NcmClient();
      final root = await Directory.systemTemp.createTemp('nsnc_playback_test_');
      final audioStore = await AudioStore.open(
        rootDirectory: root,
        maxCacheBytes: 1024 * 1024,
      );
      final player = PlayerService(client: client, audioStore: audioStore);
